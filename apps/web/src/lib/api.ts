...
export async function runWhatIf(payload: {
  ...
  const res = await fetch(`${API_BASE}/api/scenarios/what-if`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.error || `What-if failed (${res.status})`);
  }
  return res.json();
}

export async function downloadReportMarkdown(payload: {
  companyName: string;
  current: unknown;
  dataQuality?: number;
}) {
  const res = await fetch(`${API_BASE}/api/reports/markdown`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
